...
const SenegalFlagCorner = () => <svg viewBox="0 0 140 140" className="absolute left-0 top-0 h-[35mm] w-[35mm]" aria-label="Drapeau du Sénégal"><polygon points="0,0 58,0 0,58" fill="#00853F" /><polygon points="58,0 96,0 0,96 0,58" fill="#FCD116" /><polygon points="96,0 140,0 0,140 0,96" fill="#E31B23" /><text x="26" y="50" fill="#00853F" fontSize="25">★</text></svg>;
...
export const FederalOfficialCard: React.FC<FederalOfficialCardProps> = ({ official, side }) => {
  ...
  return (
    <div className="w-[85.6mm] h-[54mm] relative overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 print:rounded-none print:shadow-none">
      <SenegalFlagCorner />
      <div className="relative z-10 h-[13mm] bg-[#00853F] pl-[19mm] pr-4 pt-4 text-white"><p className="text-[9px] font-black uppercase leading-none tracking-tight">Fédération Sénégalaise de Surf</p><p className="mt-1 text-[6.5px] font-bold uppercase tracking-[0.12em] text-white/80">Carte professionnelle · cadre fédéral</p></div>
      <div className="relative z-10 flex h-[calc(54mm-13mm-10mm)] items-center gap-4 bg-white px-5 py-3"><div className="min-w-0 flex-1 self-start pt-[6mm] text-center"><h2 className="whitespace-nowrap text-[12px] uppercase leading-none tracking-tight text-slate-950"><span className="font-medium">{official.firstName} </span><span className="font-black">{official.lastName}</span></h2><AutoFitText maxFontSize={13} minFontSize={7} className="mt-3 font-black leading-tight tracking-wide text-slate-950 uppercase">{official.title}</AutoFitText><div className="mt-5 flex justify-center"><PartnerMarks /></div></div><div className="w-[25mm] shrink-0 self-center"><div className="h-[25mm] overflow-hidden bg-slate-100 shadow-md">{official.photoUrl ? <img src={official.photoUrl} alt={`${official.firstName} ${official.lastName}`} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[7px] font-bold uppercase text-slate-300">Photo</div>}</div></div></div>
