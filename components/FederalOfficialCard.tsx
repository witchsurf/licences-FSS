...
export const FederalOfficialCard: React.FC<FederalOfficialCardProps> = ({ official, side }) => {
  ...
  return (
    <div className="w-[85.6mm] h-[54mm] relative overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 print:rounded-none print:shadow-none">
      <div className="absolute -left-8 -top-8 h-36 w-36 -rotate-45 bg-[#00853F]" /><div className="absolute -left-2 -top-8 h-36 w-9 -rotate-45 bg-[#FCD116]" /><div className="absolute left-7 top-3 z-10 text-lg text-[#00853F]">★</div><div className="absolute left-7 top-0 h-36 w-9 -rotate-45 bg-[#E31B23]" />
      ...
      <div className="relative z-10 flex h-[calc(54mm-10mm)] items-center gap-4 px-7 pb-2 pt-8"><div className="min-w-0 flex-1 text-center"><AutoFitText maxFontSize={21} minFontSize={8} className="font-black leading-none tracking-tight text-slate-950 uppercase">{`${official.firstName} ${official.lastName}`}</AutoFitText><AutoFitText maxFontSize={13} minFontSize={7} className="mt-3 font-black leading-tight tracking-wide text-slate-950 uppercase">{official.title}</AutoFitText><div className="mt-5 flex justify-center"><PartnerMarks /></div></div><div className="w-[25mm] shrink-0 self-center"><div className="mb-2 flex justify-end"><Logo className="h-11 w-11" /></div><div className="h-[30mm] overflow-hidden border-2 border-sky-400 bg-slate-100 shadow-md">{official.photoUrl ? <img src={official.photoUrl} alt={`${official.firstName} ${official.lastName}`} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[7px] font-bold uppercase text-slate-300">Photo</div>}</div></div></div>
