...
export const FederalOfficialCard: React.FC<FederalOfficialCardProps> = ({ official, side }) => {
  ...
  return (
    <div className="w-[85.6mm] h-[54mm] relative overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 print:rounded-none print:shadow-none">
      ...
      <div className="absolute right-[7mm] top-[5mm] z-20"><Logo className="h-10 w-10" /></div>
      <div className="relative z-10 flex h-[calc(54mm-10mm)] items-center gap-4 px-7 pb-3 pt-8"><div className="min-w-0 flex-1 self-start pt-[9mm] text-center"><h2 className="whitespace-nowrap text-[12px] uppercase leading-none tracking-tight text-slate-950"><span className="font-medium">{official.firstName} </span><span className="font-black">{official.lastName}</span></h2><AutoFitText maxFontSize={13} minFontSize={7} className="mt-3 font-black leading-tight tracking-wide text-slate-950 uppercase">{official.title}</AutoFitText></div><div className="w-[25mm] shrink-0 self-center"><div className="h-[28mm] overflow-hidden border-2 border-sky-400 bg-slate-100 shadow-md">{official.photoUrl ? <img src={official.photoUrl} alt={`${official.firstName} ${official.lastName}`} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[7px] font-bold uppercase text-slate-300">Photo</div>}</div></div></div>
