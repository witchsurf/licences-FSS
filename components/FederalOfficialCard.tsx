...
const SenegalFlagCorner = () => <svg viewBox="0 0 140 140" className="absolute left-0 top-0 h-[35mm] w-[35mm]" aria-label="Drapeau du Sénégal"><polygon points="0,0 58,0 0,58" fill="#00853F" /><polygon points="58,0 96,0 0,96 0,58" fill="#FCD116" /><polygon points="96,0 140,0 0,140 0,96" fill="#E31B23" /><text x="30" y="45" fill="#00853F" fontSize="31">★</text></svg>;
...
export const FederalOfficialCard: React.FC<FederalOfficialCardProps> = ({ official, side }) => {
  ...
  return (
    <div className="w-[85.6mm] h-[54mm] relative overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 print:rounded-none print:shadow-none">
      <SenegalFlagCorner />
      <div className="absolute inset-0 opacity-60" style={{ backgroundImage: 'repeating-linear-gradient(172deg, transparent 0, transparent 20px, #e7f1f8 21px, transparent 23px)' }} />
      <div className="relative z-10 flex h-[calc(54mm-10mm)] items-center gap-4 px-7 pb-3 pt-8"><div className="min-w-0 flex-1 text-center"><h2 className="whitespace-nowrap text-[12px] uppercase leading-none tracking-tight text-slate-950"><span className="font-medium">{official.firstName} </span><span className="font-black">{official.lastName}</span></h2><AutoFitText maxFontSize={13} minFontSize={7} className="mt-3 font-black leading-tight tracking-wide text-slate-950 uppercase">{official.title}</AutoFitText><div className="mt-5 flex justify-center"><PartnerMarks /></div></div><div className="w-[25mm] shrink-0 self-center"><div className="mb-1 flex justify-end"><Logo className="h-9 w-9" /></div><div className="h-[22mm] overflow-hidden border-2 border-sky-400 bg-slate-100 shadow-md">{official.photoUrl ? <img src={official.photoUrl} alt={`${official.firstName} ${official.lastName}`} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[7px] font-bold uppercase text-slate-300">Photo</div>}</div></div></div>
      <div className="absolute bottom-0 left-0 right-0 h-10 bg-sky-600 pt-2 text-center text-[12px] font-black uppercase tracking-[0.1em] text-white">Fédération Sénégalaise de Surf</div>
    </div>
  );
};
