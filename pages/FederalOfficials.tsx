...
export const FederalOfficials: React.FC = () => {
  ...
  </div></div><div className="hidden print-only federal-print-grid">{officials.flatMap(official => [<FederalOfficialCard key={`${official.id}-front`} official={official} side="front" />, <FederalOfficialCard key={`${official.id}-back`} official={official} side="back" />])}</div></>;
};
